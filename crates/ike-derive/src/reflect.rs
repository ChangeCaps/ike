...
fn impl_value(name: &Ident, generics: &Generics) -> TokenStream {
    ...
    quote! {
        ...
        impl #impl_generics #reflect::FromReflect for #name #ty_generics #where_clause {
            #[inline]
        ...
        impl #impl_generics #reflect::FromReflect for #name #ty_generics #where_clause {
            ...
            fn from_reflect(reflect: &dyn #reflect::Reflect) -> Option<Self> {
                if let #reflect::ReflectRef::Struct(value) = reflect.reflect_ref() {
                    Some(Self {
                        #(
                            #field_idents: #reflect::FromReflect::from_reflect(value.field(#field_names)?)?,
                        )*
                    })
                } else {
                    None
        ...
        impl #impl_generics #reflect::FromReflect for #name #ty_generics #where_clause {
            ...
            fn from_reflect(reflect: &dyn #reflect::Reflect) -> Option<Self> {
                if let #reflect::ReflectRef::TupleStruct(value) = reflect.reflect_ref() {
                    Some(Self(
                        #(
                            #reflect::FromReflect::from_reflect(value.field(#field_indices)?)?
                        )*
                    ))
                } else {
                    None
